...
class AppMinikuraMail
{
     ...
     */
    public static function mail($to, $subject, $body, $file = null)
    {
        $confs = Configure::read('app.e.mail');
        $from_disp = Configure::read('app.e.mail.sender.MAIL FROM DISP');

        // 文面はAppConfigに置いてください
        $body = str_replace("\n", "\r\n", $body);
        ...
        // 'To'に宛先いれてください、Cc,Bccはひとまず空で良さそうです
        $headers['To'] = $to;
        $headers['Cc'] = '';
        $headers['Bcc'] = '';

        // $to が文字列なら、toのみ指定（後方互換仕様）。配列なら分解してto, cc, bcc へ入れる。
        if (is_array($to)) {
            $headers['To'] = isset($to['To']) ? $to['To'] : '';
            $headers['Cc'] = isset($to['Cc']) ? $to['Cc'] : '';
            $headers['Bcc'] = isset($to['Bcc']) ? $to['Bcc'] : '';
        }

        // 下記のsend()でSMTPの通信で必要
        $envs['RCPT TO'] = [];
        if (is_array($headers['To'])) {
            $envs['RCPT TO'] = array_merge($envs['RCPT TO'] , $headers['To']);
        } else {
            array_push($envs['RCPT TO'], $headers['To']);
        }
        if ($headers['Cc']) {
            if (is_array($headers['Cc'])) {
                $envs['RCPT TO'] = array_merge($envs['RCPT TO'] , $headers['Cc']);
            } else {
                array_push($envs['RCPT TO'], $headers['Cc']);
            }
        }
        if ($headers['Bcc']) {
            if (is_array($headers['Bcc'])) {
                $envs['RCPT TO'] = array_merge($envs['RCPT TO'] , $headers['Bcc']);
            } else {
                array_push($envs['RCPT TO'], $headers['Bcc']);
            }
        }

        if (! AppMail::send($envs, $headers, $body, $file)) {
            return false;
        }
    ...
    public static function send(array $_envs, array $_headers, $_body = null, &$_file = null, &$_msg = null)
    {
        ...
        try {
            ...
            //* Exception Trigger
            throw new Exception($exception_message, $step_code);
        }
        //* Socket Close
        fputs($sock, "QUIT\r\n");
        fclose($sock);
    ...
    /**
     *
     * @param type $to メール送信先
     * @param type $url メールに載せるパスワードリセットページへのURL
     * @return type
     */
    public static function sendResetPasswordMail($to, $url)
    {
        //configからsubject,body取得
        $subject = Configure::read('app.reset_password.mail.subject');
        $body = Configure::read('app.reset_password.mail.body');

        //body内のパスワードリセットページURLを置き換え
        $body = str_replace('%PASSWORD_RESET_URL%', $url, $body);
